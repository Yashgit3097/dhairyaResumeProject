...
// In this file, you will have utility functions that help in various tasks like validating email, formatting dates, capturing elements as images, and fixing Tailwind CSS colors.
// The `validateEmail` function checks if an email is in a valid format.
// The `inlineAllComputedStyles` function walks through a DOM element and applies all computed styles as inline styles to ensure that all colors, fonts, and other styles are applied correctly for rendering.
// The `getLightColorFromImage` function fetches an image and calculates the lightest average color from it, returning it in RGB format.
// The `formatYearMonth` function formats a date string from "YYYY-MM" to "MMM YYYY".
// The `fixTailwindColors` function replaces any unsupported OKLCH colors in a given element and its descendants with black or white, depending on the context.
// The `captureElementAsImage` function clones a DOM element, applies the `fixTailwindColors` function to it, and captures it as a PNG image using html2canvas.
// The `dataURLtoFile` function converts a data URL string into a File object, which can be useful for uploading images or storing them in a file format.
