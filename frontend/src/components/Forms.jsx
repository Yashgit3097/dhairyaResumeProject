...
import { Plus, Trash2, Sparkles, Wand2, RefreshCw, Check } from "lucide-react";
...
// ProfileInfoForm Component with Gemini AI Summary Generator
    ...
    <div className={profileInfoStyles.container}>
      ...
      <div className="space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <Input
            label="Designation / Target Job Role"
          ...
          <div className="md:col-span-2">
            <div className="flex items-center justify-between mb-2.5">
              ...
              {/* Direct Gemini AI Summary Generator Button */}
              <button
                type="button"
                onClick={() => setIsAiModalOpen(true)}
                className="group inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-gradient-to-r from-purple-600 via-indigo-600 to-blue-600 text-white text-xs font-bold shadow-md hover:shadow-purple-500/30 hover:scale-105 active:scale-95 transition-all"
              >
            ...
            <textarea
              ...
              rows={4}
              placeholder="A compelling, humanized introduction about yourself, your skills, and what value you bring..."
      ...
      <AiSummaryModal
        ...
        initialRole={profileData.designation || ""}
        onApplySummary={(generatedText) => updateSection("summary", generatedText)}
      />
    </div>
  );
};

// ProjectDetailForm Component with Gemini AI Enhancer
export const ProjectDetailForm = ({ projectInfo, updateArrayItem, addArrayItem, removeArrayItem }) => {
  const [enhancingIndex, setEnhancingIndex] = useState(null);

  const handleEnhanceProject = async (index, currentDesc, title) => {
    if (!currentDesc || !currentDesc.trim()) {
      toast.error("Please enter a short project description to enhance.");
      return;
    }

    try {
      setEnhancingIndex(index);
      const response = await axiosInstance.post(API_PATHS.AI.ENHANCE_BULLETS, {
        role: title || "Project Developer",
        text: currentDesc,
        type: "project",
      });

      if (response.data && response.data.data) {
        const bullets = response.data.data.bulletPoints;
        const formattedBullets = bullets ? bullets.map((b) => `• ${b}`).join("\n") : response.data.data.combinedParagraph;
        updateArrayItem(index, "description", formattedBullets);
        toast.success("Project description enhanced with action verbs!");
      }
    } catch (error) {
      console.error("AI Project enhance error:", error);
      const msg = error.response?.data?.message || "Failed to enhance project with AI.";
      toast.error(msg);
    } finally {
              ...
              <div className="md:col-span-2">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button
                    ...
                    onClick={() => handleEnhanceProject(index, project.description, project.title)}
                    className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-purple-50 text-purple-700 hover:bg-purple-100 text-xs font-bold border border-purple-200 transition"
                  >
                ...
                <textarea
                  placeholder="Key accomplishments, technologies used, and outcomes..."
...
// WorkExperienceForm Component with Gemini AI STAR Bullet Generator
export const WorkExperienceForm = ({ workExperience, updateArrayItem, addArrayItem, removeArrayItem }) => {
  const [enhancingIndex, setEnhancingIndex] = useState(null);

  const handleEnhanceExperience = async (index, currentDesc, role) => {
    if (!currentDesc || !currentDesc.trim()) {
      toast.error("Please enter a few notes or bullets to polish.");
      return;
    }
    ...
    try {
      ...
      if (response.data && response.data.data) {
        ...
        const formattedBullets = bullets ? bullets.map((b) => `• ${b}`).join("\n") : response.data.data.combinedParagraph;
        updateArrayItem(index, "description", formattedBullets);
        toast.success("Converted into high-impact action bullets with AI!");
      }
    } catch (error) {

      console.error("AI Experience enhance error:", error);
      const msg = error.response?.data?.message || "Failed to polish bullets with AI.";
      toast.error(msg);
    } finally {
      setEnhancingIndex(null);
    }
            ...
            <div className="mt-6">
              <div className="flex items-center justify-between mb-2">
                ...
                <button
                  type="button"
                  disabled={enhancingIndex === index}
                  onClick={() => handleEnhanceExperience(index, experience.description, experience.role)}
                  className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-purple-50 text-purple-700 hover:bg-purple-100 text-xs font-bold border border-purple-200 transition"
                >
                  {enhancingIndex === index ? (
                    <>
                      <RefreshCw className="w-3.5 h-3.5 animate-spin" />
                      <span>Writing STAR bullets...</span>
                    </>
                  ) : (
                    <>
                      <Wand2 className="w-3.5 h-3.5" />
                      <span>✨ AI Action Bullets</span>
                    </>
                  )}
                </button>
              </div>
              ...
              <textarea
                placeholder="Describe your responsibilities, metrics achieved, and tools used..."
